...
@dispatch.multi(
    # Positions
    (type[CartesianPos3D], type[CartesianPos3D], ParamsDict),
    (type[CylindricalPos], type[CylindricalPos], ParamsDict),
    (type[SphericalPos], type[SphericalPos], ParamsDict),
    (type[LonLatSphericalPos], type[LonLatSphericalPos], ParamsDict),
    (type[MathSphericalPos], type[MathSphericalPos], ParamsDict),
    (type[CartesianGeneric3D], type[CartesianGeneric3D], ParamsDict),
    # Velocities
    (type[CartesianVel3D], type[CartesianVel3D], ParamsDict),
    # Accelerations
    (type[CartesianAcc3D], type[CartesianAcc3D], ParamsDict),
)
...
def vconvert_impl(
    ...
    *,
    ...
    outp = {"r": r, "theta": theta, "phi": phi}
    outp = vconvert_parse_output(outp, to_vector.dimensions, units)
    return outp, combine_aux(in_aux, out_aux)


@dispatch.multi(
    (type[LonLatSphericalPos], type[CartesianPos3D], ParamsDict),
    (type[MathSphericalPos], type[CartesianPos3D], ParamsDict),
)
def vconvert_impl(
...
@dispatch.multi(
    (type[LonLatSphericalPos], type[CylindricalPos], ParamsDict),
    (type[MathSphericalPos], type[CylindricalPos], ParamsDict),
)
@partial(jax.jit, static_argnums=(0, 1), static_argnames=("units",))
def vconvert_impl(
    to_vector: type[AbstractSphericalPos],
...
def vconvert_impl(
    ...
    *,
    ...
    outp = {"lon": p["phi"], "lat": lat, "distance": p["r"]}
    outp = vconvert_parse_output(outp, to_vector.dimensions, units)
    return outp, combine_aux(in_aux, out_aux)
